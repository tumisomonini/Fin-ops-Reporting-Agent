...
def _step2_create_templates(pm):
...
def _step3_test_prompts(kg, pm, template1, template2):
    ...
    outcome1 = {
        "prompt": rendered1, "prompt_id": template1.id,
        "prompt_name": template1.name, "context": {},
    ...
    outcome2 = {
        "prompt": rendered2, "prompt_id": template2.id,
        "prompt_name": template2.name, "context": context,
...
def _step4_evaluate(metrics, template1, template2, outcome1, outcome2):
    print("\n📊 Step 4: Evaluating Retrievals")
    print("-"*70)
    scores1 = {"relevance": 5, "completeness": 4, "accuracy": 5, "precision": 5, "overall": 5}
    metrics.record_evaluation(
        outcome1["prompt_id"], outcome1["prompt_name"], scores1,
        {"feedback": "Excellent retrieval - got all overdue invoices"}
    )
    print(f"  ✓ Evaluated: {template1.name} - Overall: {scores1['overall']}/5")

    scores2 = {"relevance": 5, "completeness": 5, "accuracy": 5, "precision": 5, "overall": 5}
    metrics.record_evaluation(
        outcome2["prompt_id"], outcome2["prompt_name"], scores2,
        {"feedback": "Perfect - all vendor data retrieved"}
    )
    print(f"  ✓ Evaluated: {template2.name} - Overall: {scores2['overall']}/5")


def _step5_compare(comparator, outcome1, outcome2, template1, template2):
    print("\n⚖️  Step 5: Comparing Outcomes")
    print("-"*70)
    comparison = comparator.compare_outcomes(
        [outcome1, outcome2], [template1.name, template2.name]
    )
...
def _step6_show_metrics(metrics):
...
def demo_prompt_engineering():
    ...
    try:
        _step1_graph_summary(kg)
        template1, template2 = _step2_create_templates(pm)
        outcome1, outcome2 = _step3_test_prompts(kg, pm, template1, template2)
        _step4_evaluate(metrics, template1, template2, outcome1, outcome2)
        _step5_compare(comparator, outcome1, outcome2, template1, template2)
        all_metrics = _step6_show_metrics(metrics)
